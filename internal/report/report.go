...
func WriteCSVWithOptions(writer io.Writer, report benchmark.Report, options CSVOptions) error {
	...
	if err := writerCSV.Write([]string{
		...
		"corpus_mode", "corpus_zone", "corpus_nonce", "dnssec_verdict",
	}); err != nil {
		return err
	}
	for _, result := range report.Targets {
		...
		row := []string{
			...
			csvCell(report.CorpusMode), csvCell(report.CorpusZone), csvCell(report.CorpusNonce), csvCell(dnssecVerdict(result)),
		}
...
func styledStatus(status string, color bool) string {
	...
	case "RECOMMENDED", "QUALIFIED", "REFERENCE", "VALIDATING":
		colorCode = ansiGreen
	case "PROVISIONAL", "INELIGIBLE", "INCOMPLETE", "NOT COMPARABLE", "NO CLEAR DIFFERENCE", "NOT MEASURED", "INCONCLUSIVE":
